...
pub const ReleaseInfoResult = struct {
    ...
    commitsSince: u32,
    isDirty: bool,
    sourceVersion: ?[]u8, // owned by caller; null if no src/root.zig VERSION constant found (not every project uses this convention)
};

// Best-effort: not every project keeps a `pub const VERSION = "x.y.z";`
// constant in src/root.zig -- absence just means the caller can't
// cross-check it, not an error. Caught live: a self-reported VERSION
// constant was left stale across two consecutive releases (v0.73.8 and
// v0.73.10) because nothing surfaced the mismatch before tagging.
fn readSourceVersionConstant(gpa: std.mem.Allocator, io: std.Io, abs_repo_path: []const u8) ?[]u8 {
    const path = std.fs.path.join(gpa, &.{ abs_repo_path, "src", "root.zig" }) catch return null;
    defer gpa.free(path);
    const file = std.Io.Dir.openFileAbsolute(io, path, .{}) catch return null;
    defer file.close(io);
    var read_buf: [4096]u8 = undefined;
    var r = file.reader(io, &read_buf);
    // readSliceShort (not allocRemaining) deliberately -- this file is often
    // far larger than the VERSION constant's usual near-top position, and
    // allocRemaining's .limited() errors on files bigger than the cap rather
    // than truncating, which silently swallowed every real lookup here until
    // caught live against this repo's own 600KB+ root.zig.
    const dest = gpa.alloc(u8, 8192) catch return null;
    defer gpa.free(dest);
    const n = r.interface.readSliceShort(dest) catch return null;
    ...
    return gpa.dupe(u8, content[start..end]) catch null;
}
...
pub fn computeReleaseInfo(gpa: std.mem.Allocator, io: std.Io, repo_path: []const u8) !ReleaseInfoResult {
    ...
    // A failed `git status` must never silently read as "clean" -- isDirty
    // gates whether /release and /brew-release proceed, and this exact
    // false-clean shape (swallowed failure -> false) is the class of bug
    // M52 fixed in public-release-check. Propagate it as a real error,
    // same as the rev-parse HEAD check above in this same function.
    const status_raw = try runGit(gpa, io, repo_path, &.{ "status", "--porcelain" });
    defer gpa.free(status_raw);
    const is_dirty = status_raw.len > 0;
    ...
    const source_version = readSourceVersionConstant(gpa, io, repo_path);
...
// by an internal CI log-analysis tool's Jenkins-log pattern library, but
// works for any log + any pattern file.
...
// --- deep-scan subcommand ---
// Composes three already-built capabilities into a flagged-candidate list --
// no new scanning algorithm: computeContextScan's topFiles (large files
// accumulate complexity/bugs disproportionately), computeSecretScan's real
// findings, and a computeGrep sweep for TODO/FIXME markers (the standard,
// proven "flag this" heuristic). For each candidate, computeOutline gives a
// structural summary (function/class map, not full content) so Claude has
// real context to judge relevance from without reading everything blind.
//
// Session-to-session dedup: ~/.4orman/deep-scan-seen.json remembers each
// flagged file's content hash (reusing sha256Hex, the same function ledger
// IDs already use). A candidate whose content hash is unchanged since it was
// last flagged is dropped from the result -- otherwise the exact same
// unresolved TODO would re-surface and get re-reviewed every single session
// forever. A file that changes (or is flagged for the first time) always
// reappears.

pub const DeepScanCandidate = struct {
    path: []u8, // absolute
    reason: []u8,
    outline: []u8, // "" if the file's language isn't outline-supported
};

pub const DeepScanResult = struct {
    project: []u8,
    scanned_file_count: u32,
    candidates: []DeepScanCandidate, // only new-or-changed since last scan of this project
};

pub fn freeDeepScanResult(gpa: std.mem.Allocator, r: DeepScanResult) void {
    gpa.free(r.project);
    for (r.candidates) |c| {
        gpa.free(c.path);
        gpa.free(c.reason);
...
const SeenEntry = struct { key: []const u8, hash: []const u8 };
const SeenFile = struct { entries: []SeenEntry };

fn emptySeenFile(gpa: std.mem.Allocator) std.json.Parsed(SeenFile) {
    const a = gpa.create(std.heap.ArenaAllocator) catch unreachable;
    a.* = std.heap.ArenaAllocator.init(gpa);
    return .{ .arena = a, .value = .{ .entries = &.{} } };
}

fn readSeenFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8) std.json.Parsed(SeenFile) {
    // The empty fallback is only ever constructed on the path that actually
    // needs it -- constructing it unconditionally up front (even on the
    // success path, where it would just be discarded) leaked one arena per
    // call. Caught live via a Debug-build run against a real project.
    const file = std.Io.Dir.openFileAbsolute(io, path, .{}) catch return emptySeenFile(gpa);
    defer file.close(io);
    var read_buf: [4096]u8 = undefined;
    var r = file.reader(io, &read_buf);
    const content = r.interface.allocRemaining(gpa, .limited(20 * 1024 * 1024)) catch return emptySeenFile(gpa);
    defer gpa.free(content);
    return std.json.parseFromSlice(SeenFile, gpa, content, .{ .allocate = .alloc_always }) catch emptySeenFile(gpa);
}

fn writeSeenFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8, entries: []const SeenEntry) void {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(gpa);
    out.appendSlice(gpa, "{\"entries\":[") catch return;
    for (entries, 0..) |e, i| {
        if (i > 0) out.appendSlice(gpa, ",") catch return;
        const key_esc = allocJsonEscape(gpa, e.key) catch return;
        defer gpa.free(key_esc);
        const hash_esc = allocJsonEscape(gpa, e.hash) catch return;
        defer gpa.free(hash_esc);
        const entry_json = std.fmt.allocPrint(gpa, "{{\"key\":\"{s}\",\"hash\":\"{s}\"}}", .{ key_esc, hash_esc }) catch return;
        defer gpa.free(entry_json);
        out.appendSlice(gpa, entry_json) catch return;
    }
    out.appendSlice(gpa, "]}") catch return;

    const home_ptr = std.c.getenv("HOME") orelse return;
    const home = std.mem.sliceTo(home_ptr, 0);
    const dir = std.fmt.allocPrint(gpa, "{s}/.4orman", .{home}) catch return;
    defer gpa.free(dir);
...
fn outlineSummary(gpa: std.mem.Allocator, io: std.Io, abs_path: []const u8) ![]u8 {
    const o = computeOutline(gpa, io, abs_path) catch return gpa.dupe(u8, "");
    defer {
        gpa.free(o.path);
        for (o.symbols) |s| gpa.free(s.name);
        gpa.free(o.symbols);
    }
    var buf: std.ArrayList(u8) = .empty;
    errdefer buf.deinit(gpa);
    const max_syms = @min(o.symbols.len, 10);
    for (o.symbols[0..max_syms], 0..) |s, i| {
        if (i > 0) try buf.appendSlice(gpa, "; ");
        const piece = try std.fmt.allocPrint(gpa, "{s} {s} (line {d})", .{ s.kind, s.name, s.line });
...
pub fn computeDeepScan(gpa: std.mem.Allocator, io: std.Io, project_path: []const u8) !DeepScanResult {
    const home_ptr = std.c.getenv("HOME") orelse return error.NoHome;
    const home = std.mem.sliceTo(home_ptr, 0);
    const seen_path = try std.fmt.allocPrint(gpa, "{s}/.4orman/deep-scan-seen.json", .{home});
    defer gpa.free(seen_path);
    var seen_parsed = readSeenFile(gpa, io, seen_path);
    defer seen_parsed.deinit();

    const ctx = try computeContextScan(gpa, io, project_path);
    defer {
        if (ctx.entryPoint) |ep| gpa.free(ep);
        for (ctx.topFiles) |f| gpa.free(f.path);
        gpa.free(ctx.topFiles);
        for (ctx.keyFiles) |f| gpa.free(f);
        gpa.free(ctx.keyFiles);
        for (ctx.dirs) |d| gpa.free(d);
        gpa.free(ctx.dirs);
    }

    const secrets = computeSecretScan(gpa, io, project_path) catch SecretScanResult{ .findings = &.{}, .truncated = false };
    defer {
        for (secrets.findings) |f| gpa.free(f.file);
        gpa.free(secrets.findings);
    }

    const todo_hits = computeGrep(gpa, io, project_path, "TODO", null) catch GrepResult{ .pattern = "TODO", .matchCount = 0, .capped = false, .matches = &.{} };
    defer {
        for (todo_hits.matches) |m| {
            gpa.free(m.file);
            gpa.free(m.text);
        }
        gpa.free(todo_hits.matches);
    }
    const fixme_hits = computeGrep(gpa, io, project_path, "FIXME", null) catch GrepResult{ .pattern = "FIXME", .matchCount = 0, .capped = false, .matches = &.{} };
    defer {
        for (fixme_hits.matches) |m| {
            gpa.free(m.file);
            gpa.free(m.text);
        }
        gpa.free(fixme_hits.matches);
    }

    var candidates: std.ArrayList(DeepScanCandidate) = .empty;
    errdefer {
    ...
    var new_seen: std.ArrayList(SeenEntry) = .empty;
    defer {
        for (new_seen.items) |e| {
            gpa.free(e.key);
            gpa.free(e.hash);
        }
        new_seen.deinit(gpa);
    ...
    const scanned_file_count: u32 = ctx.fileCount;

    const Source = struct { path: []const u8, reason: []const u8 };
    var sources: std.ArrayList(Source) = .empty;
    defer {
        for (sources.items) |s| gpa.free(s.reason);
        sources.deinit(gpa);
    }
    for (ctx.topFiles) |f| {
        try sources.append(gpa, .{ .path = f.path, .reason = try std.fmt.allocPrint(gpa, "large file ({d} bytes)", .{f.bytes}) });
    }
    for (secrets.findings) |f| {
        try sources.append(gpa, .{ .path = f.file, .reason = try std.fmt.allocPrint(gpa, "secret-scan finding: {s} ({s} severity)", .{ f.pattern, f.severity }) });
    }
    for (todo_hits.matches) |m| {
        try sources.append(gpa, .{ .path = m.file, .reason = try std.fmt.allocPrint(gpa, "TODO marker at line {d}", .{m.line}) });
    }
    for (fixme_hits.matches) |m| {
        try sources.append(gpa, .{ .path = m.file, .reason = try std.fmt.allocPrint(gpa, "FIXME marker at line {d}", .{m.line}) });
    }

    for (sources.items) |src| {
        // context-scan/grep/secret-scan all report paths relative to the
        // scanned root, not absolute -- join them here before any file I/O
        // (openFileAbsolute/computeOutline both require an absolute path).
        const abs_candidate_path = try std.fmt.allocPrint(gpa, "{s}/{s}", .{ project_path, src.path });
        errdefer gpa.free(abs_candidate_path);

        var already_added = false;
        for (candidates.items) |c| {
            if (std.mem.eql(u8, c.path, abs_candidate_path)) {
                already_added = true;
                break;
            }
        }
        if (already_added) {
            gpa.free(abs_candidate_path);
            continue;
        }

        const content = std.Io.Dir.openFileAbsolute(io, abs_candidate_path, .{}) catch {
            gpa.free(abs_candidate_path);
            continue;
        };
        var read_buf: [4096]u8 = undefined;
        var cr = content.reader(io, &read_buf);
        const bytes = cr.interface.allocRemaining(gpa, .limited(10 * 1024 * 1024)) catch {
            content.close(io);
            gpa.free(abs_candidate_path);
            continue;
        };
        ...
        const key = try std.fmt.allocPrint(gpa, "{s}::{s}", .{ project_path, src.path });
        errdefer gpa.free(key);

        var unchanged = false;
        for (seen_parsed.value.entries) |e| {
            if (std.mem.eql(u8, e.key, key) and std.mem.eql(u8, e.hash, &hash)) {
                unchanged = true;
                break;
            }
        }

        try new_seen.append(gpa, .{ .key = key, .hash = try gpa.dupe(u8, &hash) });
        ...
        const outline = try outlineSummary(gpa, io, abs_candidate_path);
        try candidates.append(gpa, .{
            .path = abs_candidate_path,
            .reason = try gpa.dupe(u8, src.reason),
            .outline = outline,
        });
    }

    // Merge new_seen into the existing seen entries (new/changed files
    // update their hash; anything not touched this scan is carried over
    // unchanged) and persist -- next scan's dedup depends on this.
    var merged: std.ArrayList(SeenEntry) = .empty;
    defer merged.deinit(gpa);
    for (seen_parsed.value.entries) |e| {
        var superseded = false;
        for (new_seen.items) |ne| {
            if (std.mem.eql(u8, ne.key, e.key)) {
                superseded = true;
                break;
            }
        }
        if (!superseded) try merged.append(gpa, e);
    }
    for (new_seen.items) |ne| try merged.append(gpa, ne);
    writeSeenFile(gpa, io, seen_path, merged.items);

    return .{
        .project = try gpa.dupe(u8, project_path),
        .scanned_file_count = scanned_file_count,
        .candidates = try candidates.toOwnedSlice(gpa),
    };
}

// --- session coordination ---
// Confirmed live (2026-07-06): two independent Claude sessions built the
// same SessionStart hook and the same delegation-check subcommand on the
// same day, with zero coordination, wasting real reconciliation time. This
// tracks "a session is active in repo X" so a SessionStart hook can warn
// when another session is already working in the same repo.
//
// SessionEnd is not reliable as the sole cleanup signal -- confirmed via
// Claude Code's own docs and issue tracker: SIGKILL and terminal-close can
// bypass it entirely (unlike Stop, which fires reliably every turn). TTL is
// the real safety net; session-unregister is a best-effort fast path on top
// of it, not a replacement for it.
const SESSION_TTL_SECS: i64 = 900; // 15 min of no heartbeat -> treated as abandoned
...
const SessionsFile = struct { entries: []SessionEntry };

fn emptySessionsFile(gpa: std.mem.Allocator) std.json.Parsed(SessionsFile) {
    const a = gpa.create(std.heap.ArenaAllocator) catch unreachable;
    a.* = std.heap.ArenaAllocator.init(gpa);
    return .{ .arena = a, .value = .{ .entries = &.{} } };
}

fn readSessionsFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8) std.json.Parsed(SessionsFile) {
    const file = std.Io.Dir.openFileAbsolute(io, path, .{}) catch return emptySessionsFile(gpa);
    defer file.close(io);
    var read_buf: [4096]u8 = undefined;
    var r = file.reader(io, &read_buf);
    const content = r.interface.allocRemaining(gpa, .limited(4 * 1024 * 1024)) catch return emptySessionsFile(gpa);
    defer gpa.free(content);
    return std.json.parseFromSlice(SessionsFile, gpa, content, .{ .allocate = .alloc_always }) catch emptySessionsFile(gpa);
}

fn writeSessionsFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8, entries: []const SessionEntry) void {
    var out: std.ArrayList(u8) = .empty;
    defer out.deinit(gpa);
    out.appendSlice(gpa, "{\"entries\":[") catch return;
    for (entries, 0..) |e, i| {
        if (i > 0) out.appendSlice(gpa, ",") catch return;
        const id_esc = allocJsonEscape(gpa, e.session_id) catch return;
        defer gpa.free(id_esc);
        const repo_esc = allocJsonEscape(gpa, e.repo) catch return;
        defer gpa.free(repo_esc);
        const entry_json = std.fmt.allocPrint(gpa, "{{\"session_id\":\"{s}\",\"repo\":\"{s}\",\"started_at\":{d},\"last_heartbeat\":{d}}}", .{ id_esc, repo_esc, e.started_at, e.last_heartbeat }) catch return;
        defer gpa.free(entry_json);
        out.appendSlice(gpa, entry_json) catch return;
    }
    out.appendSlice(gpa, "]}") catch return;
...
fn sessionsFilePath(gpa: std.mem.Allocator) ![]u8 {
    const home_ptr = std.c.getenv("HOME") orelse return error.NoHome;
    const home = std.mem.sliceTo(home_ptr, 0);
    return std.fmt.allocPrint(gpa, "{s}/.4orman/active-sessions.json", .{home});
}

pub const OtherActiveSession = struct {
    session_id: []u8,
    started_at: i64,
    minutes_since_heartbeat: i64,
};

pub const SessionHeartbeatResult = struct {
    repo: []u8,
    session_id: []u8,
    other_active: []OtherActiveSession,
};

pub fn freeSessionHeartbeatResult(gpa: std.mem.Allocator, r: SessionHeartbeatResult) void {
    gpa.free(r.repo);
    gpa.free(r.session_id);
    for (r.other_active) |o| gpa.free(o.session_id);
    gpa.free(r.other_active);
}

// Registers/refreshes `session_id`'s entry for `repo_path` (upsert, keyed on
// the pair -- one session can hold entries for more than one repo if it cd's
// around) and returns every OTHER non-stale session active on the same
// repo. Call at SessionStart (first heartbeat) and on every Stop (refresh) --
// Stop firing every turn is what makes the TTL meaningful instead of
// expiring a still-active, merely-quiet session.
pub fn computeSessionHeartbeat(gpa: std.mem.Allocator, io: std.Io, repo_path: []const u8, session_id: []const u8) !SessionHeartbeatResult {
    const sessions_path = try sessionsFilePath(gpa);
    ...
    var parsed = readSessionsFile(gpa, io, sessions_path);
    defer parsed.deinit();

    var kept: std.ArrayList(SessionEntry) = .empty;
    defer {
        for (kept.items) |e| {
    ...
    var other_active: std.ArrayList(OtherActiveSession) = .empty;
    errdefer {
        for (other_active.items) |o| gpa.free(o.session_id);
        other_active.deinit(gpa);
    }

    var self_started_at: i64 = now;
    for (parsed.value.entries) |e| {
        const age = now - e.last_heartbeat;
        if (age > SESSION_TTL_SECS) continue; // prune: abandoned, no recent heartbeat
        const same_repo = std.mem.eql(u8, e.repo, repo_path);
        const same_session = std.mem.eql(u8, e.session_id, session_id);
        if (same_repo and same_session) {
            self_started_at = e.started_at; // preserve real start time; re-added fresh below
            continue;
        }
        if (same_repo) {
            try other_active.append(gpa, .{
                .session_id = try gpa.dupe(u8, e.session_id),
                .started_at = e.started_at,
                .minutes_since_heartbeat = @divTrunc(age, 60),
            });
        }
        try kept.append(gpa, .{
            .session_id = try gpa.dupe(u8, e.session_id),
            .repo = try gpa.dupe(u8, e.repo),
            .started_at = e.started_at,
    ...
    try kept.append(gpa, .{
        ...
        .repo = try gpa.dupe(u8, repo_path),
        .started_at = self_started_at,
        .last_heartbeat = now,
    });

    writeSessionsFile(gpa, io, sessions_path, kept.items);

    return .{
        .repo = try gpa.dupe(u8, repo_path),
        .session_id = try gpa.dupe(u8, session_id),
        .other_active = try other_active.toOwnedSlice(gpa),
    };
}

// Best-effort immediate cleanup on a clean SessionEnd. Not relied on as the
// only cleanup path -- see SESSION_TTL_SECS note above for why.
pub fn computeSessionUnregister(gpa: std.mem.Allocator, io: std.Io, repo_path: []const u8, session_id: []const u8) !bool {
    const sessions_path = try sessionsFilePath(gpa);
    defer gpa.free(sessions_path);
    ...
    var removed = false;
    for (parsed.value.entries) |e| {
        if (std.mem.eql(u8, e.repo, repo_path) and std.mem.eql(u8, e.session_id, session_id)) {
            removed = true;
            continue;
        }
        try kept.append(gpa, .{
    ...
    if (removed) writeSessionsFile(gpa, io, sessions_path, kept.items);
    return removed;
}
...
// --- recon-timeline subcommand ---
// Two independent, heuristic orderings of a project's files, side by side:
// "deterministic" ranks by raw import-statement count per file (fewer
// imports = more foundational, ranked earlier) -- reuses extractImportTarget,
// not a resolved dependency graph. "actual" ranks by each file's first-add
// commit timestamp via `git log --diff-filter=A --reverse`. Files never
// committed (or the path isn't a git repo) simply get a null actualRank.

pub const ReconTimelineEntry = struct {
    path: []u8, // owned
    depCount: u32,
    deterministicRank: u32,
    actualRank: ?u32,
    rankDelta: ?i64, // deterministicRank - actualRank, null if actualRank is null
};

pub const ReconTimelineResult = struct {
    entries: []ReconTimelineEntry, // owned, sorted by deterministicRank
    totalFiles: u32,
    filesWithGitHistory: u32,
};

const RECON_READ_CAP: usize = 32 * 1024;
const EpochRank = struct { idx: usize, epoch: i64 };

pub fn computeReconTimeline(gpa: std.mem.Allocator, io: std.Io, abs_root_path: []const u8) !ReconTimelineResult {
    const scan = try computeScan(gpa, io, abs_root_path);
    defer {
        for (scan.keyFiles) |f| gpa.free(f);
        gpa.free(scan.keyFiles);
        for (scan.dirMap) |d| gpa.free(d);
    ...
    var entries: std.ArrayList(ReconTimelineEntry) = .empty;
    errdefer {
    ...
    for (scan.files) |f| {
        const lang = outlineDetectLang(f.path);
        var dep_count: u32 = 0;

        const abs_file = try std.fs.path.join(gpa, &.{ abs_root_path, f.path });
        defer gpa.free(abs_file);
        if (std.Io.Dir.openFileAbsolute(io, abs_file, .{})) |file| {
            defer file.close(io);
            var read_buf: [4096]u8 = undefined;
            var r = file.reader(io, &read_buf);
            const dest = try gpa.alloc(u8, RECON_READ_CAP);
            defer gpa.free(dest);
            const n = r.interface.readSliceShort(dest) catch 0;
            const content = dest[0..n];
            var lines = std.mem.splitScalar(u8, content, '\n');
            while (lines.next()) |line| {
                if (try extractImportTarget(gpa, lang, line)) |target| {
    ...
    std.mem.sort(ReconTimelineEntry, entries.items, {}, struct {
        fn lessThan(_: void, a: ReconTimelineEntry, b: ReconTimelineEntry) bool {
            if (a.depCount != b.depCount) return a.depCount < b.depCount;
            return std.mem.lessThan(u8, a.path, b.path);
        }
    }.lessThan);
    for (entries.items, 0..) |*e, i| e.deterministicRank = @intCast(i);

    var first_commit: std.StringHashMap(i64) = .init(gpa);
    defer {
        var it = first_commit.keyIterator();
        while (it.next()) |k| gpa.free(k.*);
        first_commit.deinit();
    }

    if (runGit(gpa, io, abs_root_path, &.{ "log", "--diff-filter=A", "--name-only", "--format=COMMIT:%ct", "--reverse" })) |raw| {
        defer gpa.free(raw);
        var current_epoch: ?i64 = null;
        var lines = std.mem.splitScalar(u8, raw, '\n');
        while (lines.next()) |raw_line| {
            const line = std.mem.trim(u8, raw_line, " \t\r");
            if (line.len == 0) continue;
            if (std.mem.startsWith(u8, line, "COMMIT:")) {
                current_epoch = std.fmt.parseInt(i64, line["COMMIT:".len..], 10) catch null;
                continue;
            }
            if (current_epoch) |epoch| {
                if (!first_commit.contains(line)) {
                    try first_commit.put(try gpa.dupe(u8, line), epoch);
                }
            }
        }
    } else |_| {}

    var with_epoch: std.ArrayList(EpochRank) = .empty;
    defer with_epoch.deinit(gpa);
    for (entries.items, 0..) |e, i| {
        if (first_commit.get(e.path)) |epoch| {
            try with_epoch.append(gpa, .{ .idx = i, .epoch = epoch });
        }
    }
    std.mem.sort(EpochRank, with_epoch.items, {}, struct {
        fn lessThan(_: void, a: EpochRank, b: EpochRank) bool {
            return a.epoch < b.epoch;
        }
    }.lessThan);
    for (with_epoch.items, 0..) |w, rank| {
        entries.items[w.idx].actualRank = @intCast(rank);
        entries.items[w.idx].rankDelta = @as(i64, @intCast(entries.items[w.idx].deterministicRank)) - @as(i64, @intCast(rank));
    }

    return .{
        .entries = try entries.toOwnedSlice(gpa),
        .totalFiles = @intCast(scan.files.len),
        .filesWithGitHistory = @intCast(with_epoch.items.len),
    };
}
...
// Short TTL, not a long one: tool versions genuinely can't change within a
// single invocation, but they CAN change between two invocations if the user
// runs `brew upgrade` in between -- and the whole point of compat-check is
// to catch that promptly in the next fresh terminal. A short window still
// eliminates the redundant subprocess-spawn burst from opening several
// terminal tabs in quick succession at the start of a work session, without
// meaningfully widening the chance of masking a genuine just-upgraded tool.
// A values tradeoff with no single provably-correct number -- recorded via
// ledger record-jungian rather than presented as a measured optimum.
const COMPAT_CHECK_CACHE_TTL_SECS: i64 = 5 * 60;

fn compatCheckCachePath(gpa: std.mem.Allocator) !?[]u8 {
    const home_ptr = std.c.getenv("HOME") orelse return null;
    const home = std.mem.sliceTo(home_ptr, 0);
    return try std.fmt.allocPrint(gpa, "{s}/.4orman/compat-check-cache.json", .{home});
}

fn parseIntField(content: []const u8, field: []const u8) ?i64 {
    var lines = std.mem.splitScalar(u8, content, '\n');
    while (lines.next()) |line| {
        const trimmed = std.mem.trim(u8, line, " \t\r");
        if (field.len + 3 > 60) continue;
        ...
        const key = key_buf[0 .. 3 + field.len];
        const pos = std.mem.indexOf(u8, trimmed, key) orelse continue;
        const rest = std.mem.trimStart(u8, trimmed[pos + key.len ..], " \t");
...
// Returns null on any cache miss (missing file, unparseable, or past TTL) --
// the caller falls back to a fresh full check on any of these, never treats
// a miss as "versions unknown."
fn loadFreshCompatCacheTools(gpa: std.mem.Allocator, io: std.Io, cache_path: []const u8) ?[COMPAT_TOOLS.len][]u8 {
    const f = std.Io.Dir.openFileAbsolute(io, cache_path, .{}) catch return null;
    defer f.close(io);
    var rbuf: [4096]u8 = undefined;
    var r = f.reader(io, &rbuf);
    const content = r.interface.allocRemaining(gpa, .limited(16 * 1024)) catch return null;
    defer gpa.free(content);

    const cached_at = parseIntField(content, "cached_at_epoch") orelse return null;

    var ts: std.c.timespec = undefined;
    _ = std.c.clock_gettime(std.c.CLOCK.REALTIME, &ts);
    // A clock that went backward is treated the same as "too old": never
    // trust a cache entry we can't confidently place inside the TTL window.
    if (ts.sec < cached_at or ts.sec - cached_at > COMPAT_CHECK_CACHE_TTL_SECS) return null;

    var out: [COMPAT_TOOLS.len][]u8 = undefined;
    var n_filled: usize = 0;
    for (COMPAT_TOOLS, 0..) |tool, i| {
        out[i] = gpa.dupe(u8, parseBaselineField(content, tool)) catch {
            for (out[0..n_filled]) |v| gpa.free(v);
            return null;
        };
        n_filled = i + 1;
    }
...
fn writeCompatCheckCache(gpa: std.mem.Allocator, io: std.Io, cache_path: []const u8, versions: [COMPAT_TOOLS.len][]const u8) !void {
    var ts: std.c.timespec = undefined;
    _ = std.c.clock_gettime(std.c.CLOCK.REALTIME, &ts);
    ...
    f.close(io);
    if (ok) atomicRenameAbsolute(tmp_path, cache_path);
}

// Session-lifetime cache for the expensive part of compat-check: spawning a
// subprocess per tool to read its current version. A cache hit replaces up
// to 6 subprocess spawns with a single file read; a miss falls back to the
// exact same per-tool subprocess calls this function always used, then
// best-effort writes the result so the next call within the TTL is a hit.
// Never fails the caller if the cache itself can't be read or written --
// caching is a pure optimization, not a correctness dependency.
fn getCurrentVersionsCached(gpa: std.mem.Allocator, io: std.Io) ![COMPAT_TOOLS.len][]u8 {
    const cache_path = (compatCheckCachePath(gpa) catch null);
    defer if (cache_path) |p| gpa.free(p);

    if (cache_path) |p| {
        if (loadFreshCompatCacheTools(gpa, io, p)) |cached| return cached;
    }

    var out: [COMPAT_TOOLS.len][]u8 = undefined;
    var n_filled: usize = 0;
    errdefer for (out[0..n_filled]) |v| gpa.free(v);
    for (COMPAT_TOOLS, 0..) |tool, i| {
        out[i] = try getToolVersionAlloc(gpa, io, tool);
        n_filled = i + 1;
    }

    if (cache_path) |p| writeCompatCheckCache(gpa, io, p, out) catch {};
...
pub fn computeCompatCheck(gpa: std.mem.Allocator, io: std.Io) !CompatCheckResult {
    ...
    const now_versions = try getCurrentVersionsCached(gpa, io);
    defer for (now_versions) |v| gpa.free(v);
    ...
    for (COMPAT_TOOLS, 0..) |tool, i| {
        ...
        const now_str = now_versions[i];
        if (std.mem.eql(u8, was_str, now_str)) continue;
...
// Harness-tag-shaped or injected-instruction phrases planted in tracked file
// content, hoping a future agent reading the file mistakes them for a real
// instruction. Deliberately NOT gated by secretScanIsCommentLine below --
// comments and docs are exactly where planted text would hide, unlike real
// secrets which the existing SECRET_PATTERNS loop correctly deprioritizes
// there. Case-insensitive, plain substring -- no continuation-length gate,
// since presence alone is the signal, not what follows it.
const PROMPT_INJECTION_MARKERS = [_][]const u8{
    "<system-reminder>",
    "<system_prompt>",
...
// A line that's just a bare quoted string (optionally trailing comma) is the
// PROMPT_INJECTION_MARKERS array's own definition, not planted content --
// real injected text lives inside surrounding prose/markup, never as an
// isolated Zig array element. Known gap: a marker assembled as a standalone
// quoted JSON string value (no surrounding text on the same line) would also
// be exempted by this same check -- same class of tradeoff secret-scan
// already accepts via its .example/.sample/.template/.dist exemption.
fn injectionScanIsBareLiteral(line: []const u8) bool {
    const t = std.mem.trim(u8, line, " \t\r");
    if (t.len < 2 or t[0] != '"') return false;
    var i: usize = 1;
    while (i < t.len and t[i] != '"') : (i += 1) {}
    if (i >= t.len) return false;
    const rest = std.mem.trim(u8, t[i + 1 ..], " \t\r,");
    return rest.len == 0;
}
...
fn secretScanValueWasQuoted(line: []const u8, sep_pos: usize) bool {
    if (sep_pos + 1 >= line.len) return false;
    const after = std.mem.trimStart(u8, line[sep_pos + 1 ..], " \t");
    const trimmed = std.mem.trimEnd(u8, after, " \t;,\r");
    if (trimmed.len < 2) return false;
    const q = trimmed[0];
    return (q == '"' or q == '\'' or q == '`') and trimmed[trimmed.len - 1] == q;
}

// A bare identifier (letters/digits/underscore only, starting with a letter
// or underscore) is a variable/field reference, never a hardcoded secret
// literal -- real secret values only ever appear as quoted strings in
// source. Distinct from secretScanIsPlaceholder's null_literals check,
// which only catches exact "true"/"false", not arbitrary bool-named
// variables like `has_secrets` referenced under an unrelated field name
// (e.g. `.excludeSecrets = has_secrets`, where self_ref's substring check
// also can't help since neither name contains the other).
fn secretScanIsBareIdentifier(val: []const u8) bool {
    if (val.len == 0) return false;
    if (!(std.ascii.isAlphabetic(val[0]) or val[0] == '_')) return false;
    for (val) |c| {
...
fn scanFileForSecrets(
    ...
    var last_flagged_injection: u32 = 0;
    while (line_start < content.len) {
        const line_end = std.mem.indexOfScalarPos(u8, content, line_start, '\n') orelse content.len;
        const line = content[line_start..line_end];
        if (line_num != last_flagged_injection and !injectionScanIsBareLiteral(line)) {
            for (PROMPT_INJECTION_MARKERS) |marker| {
                if (findings.items.len >= SECRET_SCAN_MAX_FINDINGS) {
                    truncated.* = true;
                    return;
                }
                if (secretScanCaseInsensitiveContains(line, marker)) {
                    try findings.append(gpa, .{
                        .file = try gpa.dupe(u8, rel_path),
                        .line = line_num,
                        .pattern = "prompt-injection-marker",
                        .severity = "high",
                    });
                    last_flagged_injection = line_num;
                    break;
                }
            }
        }
                    ...
                    .assignment_key => {
                        if (secretScanFindAssignmentSep(line)) |sep_pos| {
                            ...
                            if (secretScanIsCleanKey(key) and secretScanCaseInsensitiveContains(key, pat.needle)) {
                                ...
                                // An unquoted bare identifier (e.g. `.excludeSecrets =
                                // has_secrets`) is a variable reference under a
                                // differently-named field -- self_ref's substring check
                                // can't catch this since neither name contains the other,
                                // but a real secret literal is never unquoted source code.
                                const quoted = secretScanValueWasQuoted(line, sep_pos);
                                const bare_ref = !quoted and secretScanIsBareIdentifier(val);
                                if (!secretScanIsPlaceholder(val) and !self_ref and !bare_ref) matched = true;
                            }
                        }
                    },
                }
...
fn prcTrackedFiles(gpa: std.mem.Allocator, io: std.Io, repo: []const u8) ![][]u8 {
    const raw = try runGit(gpa, io, repo, &.{"ls-files"});
    defer gpa.free(raw);
    var out: std.ArrayList([]u8) = .empty;
    errdefer {
...
pub fn computePublicReleaseCheck(gpa: std.mem.Allocator, io: std.Io, repo: []const u8, watchlist_path_arg: ?[]const u8) !PublicReleaseCheckResult {
    ...
    const tracked = prcTrackedFiles(gpa, io, repo) catch |err| blk: {
        // A git failure here (not a repo, no commits yet, etc.) must never be
        // silently treated as "zero tracked files" -- that made every check
        // below scan nothing and report ready:true on a directory full of
        // unscanned real content (confirmed live: a plain non-git scratch
        // copy full of leaks reported clean). Surface it as a blocking
        // finding instead of swallowing it.
        try findings.append(gpa, .{
            .check = "structure",
            .severity = "high",
            .file = "",
            .line = 0,
            .detail = try std.fmt.allocPrint(gpa, "could not list tracked files ({s}) -- target is likely not a git repository yet (run `git init` + `git add` first); zero files were scanned", .{@errorName(err)}),
        });
        break :blk &[_][]u8{};
    };
    defer {
...
pub fn computeRegistry() RegistryResult {
    const cmds: []const RegistrySubcommand = &[_]RegistrySubcommand{
        ...
        .{ .name = "recon-timeline", .description = "deterministic (import-count) vs actual (git first-commit) file ordering, side by side", .args = "<path>" },
        ...
        .{ .name = "deep-scan", .description = "composite structural pass (large files, secret-scan hits, TODO/FIXME markers) feeding candidates into the bug/feature ledger", .args = "<abs-path>" },
        .{ .name = "session-heartbeat", .description = "register/refresh this session's presence in a repo and return every other non-stale session active on it (15-min TTL) -- detects concurrent-session duplicate work", .args = "<repo-path> <session-id>" },
        .{ .name = "session-unregister", .description = "best-effort immediate cleanup of a session's presence entry on clean exit -- not the sole cleanup path, session-heartbeat's TTL is the real safety net", .args = "<repo-path> <session-id>" },
        .{ .name = "names-check", .description = "domain-agnostic doc-vs-reality drift check -- which of a caller-supplied list of names is missing (whole-word) from a doc file; e.g. command files vs help.md, or a registry dump vs CLAUDE.md", .args = "<doc-path> <comma-separated-names>" },
        ...
        .{ .name = "ledger", .description = "decision ledger — Claude-vs-Zig contests, values trade-offs, solo-vs-multi-agent approach decisions, ROI-ranked curriculum entries, gated architecture-change proposals, and a shared bug/feature ticket tracker, with 365-day staleness revalidation (stored at ~/.4orman/ledger.json)", .args = "[show | record <winner> <question> <reasoning> | record-jungian <question> <chosen> <shadow> <synthesis> | record-outcome <id> <outcome> <matched|diverged> | record-approach <question> <solo|multi-agent> <reasoning> <tokens-spent> | record-curriculum <domain> <topic> <highest|medium|lowest> <tool|doc|both> <artifact-ref> <reasoning> <shadow> <synthesis> | record-architecture <topic> <highest|medium|lowest> <pct> <true|false> <proposed|built|rejected> <reasoning> <shadow> <synthesis> | record-bug <project> <error-signature> <fix> [fixed|open] [vendor-name] | find-bug <query> [threshold] [max-results] | record-feature <domain> <idea-summary> <ready|needs-input|deferred> <tool|doc|project|none> <artifact-ref> <reasoning> <shadow> <synthesis> | check-precedent <question> | check-stale | review-due | validate <id> | score <question> <sources-json>]" },
...
// Standalone CLI query over findLedgerPrecedent — until this existed, the
// only way to check for an rps-category precedent was to read the whole
// `ledger show` output and eyeball it. Same 50%-word-overlap match, same
// jungian/approach/curriculum/architecture exclusion, same staleness
// filter findLedgerPrecedent already enforces; this just exposes it
// directly instead of embedding it inside build/run-tests/capability-check.
pub fn computeLedgerCheckPrecedent(gpa: std.mem.Allocator, io: std.Io, query: []const u8) ![]u8 {
    const query_esc = try allocJsonEscape(gpa, query);
    ...
    if (try findLedgerPrecedent(gpa, io, query)) |entry| {
        ...
        const id_esc = try allocJsonEscape(gpa, entry.id);
        defer gpa.free(id_esc);
        const question_esc = try allocJsonEscape(gpa, entry.question);
        defer gpa.free(question_esc);
        const winner_esc = try allocJsonEscape(gpa, entry.winner);
        defer gpa.free(winner_esc);
        const reasoning_esc = try allocJsonEscape(gpa, entry.reasoning);
        defer gpa.free(reasoning_esc);
        const date_esc = try allocJsonEscape(gpa, entry.date);
        defer gpa.free(date_esc);
        return std.fmt.allocPrint(gpa,
            ...
            \\  "date": "{s}"
            \\}}
        , .{ query_esc, id_esc, question_esc, winner_esc, reasoning_esc, date_esc });
    }
...
pub const BugMatch = struct {
    entry: LedgerEntry,
    score: u32, // 0-100, ledgerWordOverlapScore against the stored error_signature
};

pub const BugFindResult = struct {
    matches: []BugMatch,
};

pub fn freeBugFindResult(gpa: std.mem.Allocator, r: BugFindResult) void {
    for (r.matches) |m| {
        gpa.free(m.entry.id);
        gpa.free(m.entry.date);
        gpa.free(m.entry.winner);
        gpa.free(m.entry.question);
        gpa.free(m.entry.reasoning);
        gpa.free(m.entry.category);
        gpa.free(m.entry.shadow);
        gpa.free(m.entry.synthesis);
        gpa.free(m.entry.outcome);
        gpa.free(m.entry.outcome_matched);
        gpa.free(m.entry.domain);
        gpa.free(m.entry.artifact_type);
        gpa.free(m.entry.artifact_ref);
        gpa.free(m.entry.status);
    }
    gpa.free(r.matches);
}

// Sibling to findLedgerPrecedent, scoped to category == "bug" entries. Unlike
// findLedgerPrecedent (single best match, >=50% threshold tuned for repeated
// architecture/tool-choice questions), this returns every match above
// `threshold` sorted best-first, capped at `max_results` -- bug reports of the
// same underlying issue are often worded quite differently, so a lower
// default threshold (30) favors recall over precision; Claude is expected to
// read the returned fix text and judge whether it's actually the same bug,
// not treat any single match as an automatic answer. is_stale is deliberately
// not filtered here -- a fix doesn't expire the way an architecture decision
// might.
pub fn computeFindBug(gpa: std.mem.Allocator, io: std.Io, query: []const u8, threshold: u32, max_results: usize) !BugFindResult {
    const home_ptr = std.c.getenv("HOME") orelse return .{ .matches = &.{} };
    const home = std.mem.sliceTo(home_ptr, 0);
    const ledger_path = try std.fmt.allocPrint(gpa, "{s}/.4orman/ledger.json", .{home});
    defer gpa.free(ledger_path);
    ...
    var scored: std.ArrayList(BugMatch) = .empty;
    errdefer {
        for (scored.items) |m| {
            gpa.free(m.entry.id);
            gpa.free(m.entry.date);
    ...
    for (entries.items) |e| {
        if (!std.mem.eql(u8, e.category, "bug")) continue;
        const qn_lower = try gpa.alloc(u8, e.question.len);
        defer gpa.free(qn_lower);
        for (e.question, 0..) |c, i| qn_lower[i] = std.ascii.toLower(c);
    ...
    std.mem.sort(BugMatch, scored.items, {}, struct {
        fn lessThan(_: void, a: BugMatch, b: BugMatch) bool {
            return a.score > b.score;
        }
    }.lessThan);

    if (scored.items.len > max_results) {
        for (scored.items[max_results..]) |m| {
            gpa.free(m.entry.id);
            gpa.free(m.entry.date);
            gpa.free(m.entry.winner);
            gpa.free(m.entry.question);
    ...
    return .{ .matches = try scored.toOwnedSlice(gpa) };
}
...
// Domain-agnostic doc-vs-reality drift check: does `names` appear anywhere
// in `doc-path`? Deliberately not hardcoded to "commands vs help.md" or
// "registry vs CLAUDE.md" -- both of those were done by hand the same day
// via ad-hoc shell diffing, twice each, the same 2+-repeats signal that
// promotes anything else in this codebase to a permanent subcommand. The
// caller supplies whatever "expected names" list fits its own case (a
// directory listing, a registry dump, anything) -- this only does the
// whole-word presence check, reusing containsWholeWord unmodified.
pub fn computeNamesCheck(gpa: std.mem.Allocator, io: std.Io, doc_path: []const u8, names_csv: []const u8) ![]u8 {
    const file = std.Io.Dir.openFileAbsolute(io, doc_path, .{}) catch return error.DocNotFound;
    defer file.close(io);
    var read_buf: [4096]u8 = undefined;
    var r = file.reader(io, &read_buf);
    const content = try r.interface.allocRemaining(gpa, .limited(8 * 1024 * 1024));
    defer gpa.free(content);
    ...
    while (it.next()) |raw_name| {
        ...
        if (name.len == 0) continue;
        checked += 1;
        if (!containsWholeWord(content, name)) try missing.append(gpa, name);
    }

    const doc_esc = try allocJsonEscape(gpa, doc_path);
    defer gpa.free(doc_esc);

    var missing_buf: std.ArrayList(u8) = .empty;
    defer missing_buf.deinit(gpa);
    for (missing.items, 0..) |name, i| {
        if (i > 0) try missing_buf.appendSlice(gpa, ",");
        const name_esc = try allocJsonEscape(gpa, name);
        defer gpa.free(name_esc);
        try missing_buf.appendSlice(gpa, "\"");
        try missing_buf.appendSlice(gpa, name_esc);
        try missing_buf.appendSlice(gpa, "\"");
    }
    ...
    return std.fmt.allocPrint(gpa,
        ...
        \\  "missing": [{s}]
        \\}}
    , .{ doc_esc, checked, missing.items.len, missing_buf.items });
}
...
pub fn computeCapabilityCheck(gpa: std.mem.Allocator, io: std.Io, query: []const u8) !CapabilityCheckResult {
    ...
    if (try findLedgerPrecedent(gpa, io, query)) |entry| {
        // findLedgerPrecedent dupes every LedgerEntry field. Only .id and
        // .reasoning are transferred to the return value below (ownership
        // moves to the caller); every other duped field must be freed here
        // or it leaks on every ledger-hit call.
        defer {
            gpa.free(entry.date);
            gpa.free(entry.winner);
            gpa.free(entry.question);
...
// A ready-to-review vendor email, drafted only when record_bug is given a
// non-empty vendor name -- never sent, Zig has no email transport. needsFrom/
// needsTo/needsCc (printed by main.zig, not stored here) are the signal for
// Claude to ask the user for those three addresses before the draft is used.
pub const VendorEmailDraft = struct {
    subject: []u8,
    bullets: [][]u8,
    signature: []u8,
};

pub fn freeVendorEmailDraft(gpa: std.mem.Allocator, d: VendorEmailDraft) void {
    gpa.free(d.subject);
    for (d.bullets) |b| gpa.free(b);
    gpa.free(d.bullets);
    gpa.free(d.signature);
}
...
pub const LedgerResult = struct {
    ...
    stale_count: usize,
    total: usize,
    vendor_email: ?VendorEmailDraft, // non-null only for a record_bug call with a non-empty vendor
};
...
pub const LedgerMode = union(enum) {
    ...
    // A ticket for a real bug and its fix -- reuses this same LedgerEntry
    // shape rather than a new file/struct: `domain` = which orman project hit
    // it, `question` = the error signature (the fuzzy-match key), `reasoning`
    // = the fix description, `status` = "fixed" (default) or "open". No
    // staleness/outcome-review semantics apply here (a fix doesn't expire),
    // but the fields still get populated since LedgerEntry always sets them.
    // `vendor` ("" = not vendor-related) is stored via the already-unused-
    // for-bugs `artifact_type` field -- when non-empty, computeLedger also
    // drafts a VendorEmailDraft aggregating every "bug" entry sharing this
    // vendor, past and present, never just the one just filed.
    record_bug: struct { project: []const u8, error_signature: []const u8, fix: []const u8, status: []const u8, vendor: []const u8 },
    // A feature/idea brainstorm evaluated the way /new-project's spec
    // interview would, then filed for later reference -- Claude judges (fit,
    // readiness, what's uncertain), Zig only records. Same one-free-text-
    // field-per-category reuse as curriculum/architecture: `domain` = project
    // or topic name, `topic` (-> `question`) = one-line idea summary,
    // `readiness_tier` (-> `winner`) = "ready" | "needs-input" | "deferred",
    // `shadow` = the more conservative/minimal-scope alternative passed over
    // (the standing directive is to bias toward the richest feasible scope,
    // not a minimal one -- `shadow` records what was deliberately not
    // chosen), `synthesis` = what's kept at full scope vs. still uncertain.
    // `status` is derived from `artifact_ref` exactly like curriculum's.
    record_feature: struct {
        domain: []const u8,
        topic: []const u8,
        readiness_tier: []const u8,
        artifact_type: []const u8,
        artifact_ref: []const u8,
        reasoning: []const u8,
        shadow: []const u8,
    ...
    check_stale: void,
    validate: []const u8,
    // Returns only entries with outcome_review_due true, instead of every
    // caller re-fetching the full `show` dump and filtering client-side --
    // that exact filter got reimplemented at least 3 times the same day this
    // was added (once permanently in session-start-hook.py, twice more as
    // ad-hoc one-off checks), the same 2+-repeats signal that promotes
    // anything else in this codebase to a permanent subcommand.
    review_due: void,
};
...
pub fn computeLedger(gpa: std.mem.Allocator, io: std.Io, mode: LedgerMode) !LedgerResult {
    ...
    var vendor_email: ?VendorEmailDraft = null;

    switch (mode) {
        .show, .check_stale, .review_due => {},
        .record => |rec| {
            const date = try tsToDateStr(gpa, @intCast(now_ts));
            defer gpa.free(date);
        ...
        .record_bug => |rb| {
            const date = try tsToDateStr(gpa, @intCast(now_ts));
            defer gpa.free(date);
            const id_src = try std.fmt.allocPrint(gpa, "bug:{s}:{s}:{s}", .{ rb.project, rb.error_signature, date });
            defer gpa.free(id_src);
            const hex = sha256Hex(id_src);
            const entry = LedgerEntry{
                .id = try gpa.dupe(u8, hex[0..16]),
                ...
                .recorded_ts = now_ts,
                .revalidation_due_ts = now_ts + LEDGER_STALE_SECS,
                .winner = try gpa.dupe(u8, ""),
                .question = try gpa.dupe(u8, rb.error_signature),
                .reasoning = try gpa.dupe(u8, rb.fix),
                .is_stale = false,
                .category = try gpa.dupe(u8, "bug"),
                .shadow = try gpa.dupe(u8, ""),
                .synthesis = try gpa.dupe(u8, ""),
                .outcome = try gpa.dupe(u8, ""),
                .outcome_matched = try gpa.dupe(u8, "unknown"),
                ...
                .outcome_review_due = false,
                .tokens_spent = 0,
                .domain = try gpa.dupe(u8, rb.project),
                .artifact_type = try gpa.dupe(u8, rb.vendor),
                .artifact_ref = try gpa.dupe(u8, ""),
                .status = try gpa.dupe(u8, rb.status),
                .estimated_improvement_pct = 0.0,
                .has_drawbacks = false,
            };
            try entries.append(gpa, entry);
            ...
            if (rb.vendor.len > 0) {
                var bullets: std.ArrayList([]u8) = .empty;
                errdefer {
                    for (bullets.items) |b| gpa.free(b);
                    bullets.deinit(gpa);
                }
                for (entries.items) |e| {
                    if (!std.mem.eql(u8, e.category, "bug")) continue;
                    if (!std.ascii.eqlIgnoreCase(e.artifact_type, rb.vendor)) continue;
                    const bullet = if (e.reasoning.len > 0)
                        try std.fmt.allocPrint(gpa, "{s} (workaround: {s})", .{ e.question, e.reasoning })
                    else
                        try gpa.dupe(u8, e.question);
                    try bullets.append(gpa, bullet);
                }

                const subject = if (bullets.items.len > 1)
                    try std.fmt.allocPrint(gpa, "Bug Report: {s} -- {d} issue(s)", .{ rb.vendor, bullets.items.len })
                else
                    try std.fmt.allocPrint(gpa, "Bug Report: {s} -- {s}", .{ rb.vendor, rb.error_signature });

                vendor_email = VendorEmailDraft{
                    .subject = subject,
                    .bullets = try bullets.toOwnedSlice(gpa),
                    .signature = try gpa.dupe(u8, "Best regards,\n[Your Name]"),
                };
            }
        },
        .record_feature => |rf| {
            // Values/readiness call, not a factual one -- same reasoning as
            // record_curriculum. `status` is derived from `artifact_ref`
            // rather than a separate argument: a non-empty ref means
            // /new-project actually scaffolded something from this idea.
            const date = try tsToDateStr(gpa, @intCast(now_ts));
            defer gpa.free(date);
            const id_src = try std.fmt.allocPrint(gpa, "feature:{s}:{s}:{s}", .{ rf.domain, rf.topic, date });
            defer gpa.free(id_src);
            const hex = sha256Hex(id_src);
            const status_str: []const u8 = if (rf.artifact_ref.len > 0) "built" else "proposed";
            const entry = LedgerEntry{
                .id = try gpa.dupe(u8, hex[0..16]),
                .date = try gpa.dupe(u8, date),
                .recorded_ts = now_ts,
                .revalidation_due_ts = now_ts + LEDGER_STALE_SECS,
                .winner = try gpa.dupe(u8, rf.readiness_tier),
                .question = try gpa.dupe(u8, rf.topic),
                .reasoning = try gpa.dupe(u8, rf.reasoning),
                .is_stale = false,
                .category = try gpa.dupe(u8, "feature"),
                .shadow = try gpa.dupe(u8, rf.shadow),
                .synthesis = try gpa.dupe(u8, rf.synthesis),
                .outcome = try gpa.dupe(u8, ""),
                .outcome_matched = try gpa.dupe(u8, "unknown"),
                .outcome_recorded_ts = 0,
                .outcome_review_due = false,
                .tokens_spent = 0,
                .domain = try gpa.dupe(u8, rf.domain),
                .artifact_type = try gpa.dupe(u8, rf.artifact_type),
                .artifact_ref = try gpa.dupe(u8, rf.artifact_ref),
                .status = try gpa.dupe(u8, status_str),
                .estimated_improvement_pct = 0.0,
                .has_drawbacks = false,
            };
        ...
        .record_outcome => |ro| {
            ...
            for (entries.items) |*e| {
                if (std.mem.startsWith(u8, e.id, ro.id) or std.mem.eql(u8, e.id, ro.id)) {
                    ...
                    // A "bug" entry's outcome matching its recorded fix means
                    // the bug really is fixed -- keep `status` in sync so
                    // ticket-board-style views (open_bugs filters on status
                    // alone) don't keep listing a verified-fixed bug as open.
                    if (std.mem.eql(u8, e.category, "bug") and std.mem.eql(u8, ro.matched, "matched")) {
                        gpa.free(e.status);
                        e.status = try gpa.dupe(u8, "fixed");
                    }
                    found = true;
                    break;
                }
    ...
    const result_entries = switch (mode) {
        .check_stale => blk: {
            // stale_list copies entries by value -- the copy shares the same
            // []u8 field pointers as the original (a shallow copy, not a
            // deep one), so a stale entry's strings must not be freed here:
            // ownership transfers to result_entries below, and main.zig's
            // dispatch frees them once printed. A non-stale entry is never
            // returned to the caller at all, so its strings would be
            // orphaned once `entries` itself is discarded -- free them now.
            var stale_list: std.ArrayList(LedgerEntry) = .empty;
            for (entries.items) |e| {
                if (e.is_stale) {
                    try stale_list.append(gpa, e);
                } else {
                    gpa.free(e.id);
                    gpa.free(e.date);
                    gpa.free(e.winner);
            ...
            entries.deinit(gpa); // backing array only -- stale entries' strings live on via stale_list
            break :blk try stale_list.toOwnedSlice(gpa);
        },
        .review_due => blk: {
            // Same shallow-copy/selective-free shape as .check_stale above --
            // only entries with outcome_review_due=true survive to the
            // caller, everything else's owned strings must be freed here or
            // they leak once `entries` itself is discarded.
            var due_list: std.ArrayList(LedgerEntry) = .empty;
            for (entries.items) |e| {
                if (e.outcome_review_due) {
                    try due_list.append(gpa, e);
                } else {
                    gpa.free(e.id);
                    gpa.free(e.date);
                    gpa.free(e.winner);
            ...
            entries.deinit(gpa);
            break :blk try due_list.toOwnedSlice(gpa);
        },
...
test "ReleaseInfoResult fields" {
    const r = ReleaseInfoResult{ .latestTag = "v1.2.3", .suggestedNext = "v1.2.4", .commitsSince = 5, .isDirty = false, .sourceVersion = null };
    ...
    try std.testing.expect(r.sourceVersion == null);
}

test "readSourceVersionConstant: extracts VERSION from a real-shaped root.zig" {
    const gpa = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io, "src");
    try tmp.dir.writeFile(io, .{ .sub_path = "src/root.zig", .data = "const std = @import(\"std\");\n\npub const VERSION = \"9.9.9\";\n" });
    const repo_path = try tmp.dir.realPathFileAlloc(io, ".", gpa);
    defer gpa.free(repo_path);

    const v = readSourceVersionConstant(gpa, io, repo_path);
    defer if (v) |vv| gpa.free(vv);
    try std.testing.expect(v != null);
    try std.testing.expectEqualStrings("9.9.9", v.?);
}

test "readSourceVersionConstant: null when src/root.zig is absent" {
    const gpa = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "secretScanIsBareIdentifier / secretScanValueWasQuoted: unquoted bool variable under a differently-named field is not a secret" {
    // Regression: `.excludeSecrets = has_secrets,` in this file's own
    // context-gate plumbing was flagged -- self_ref's substring check
    // can't help here since "excludeSecrets" doesn't contain "has_secrets",
    // and secretScanIsPlaceholder's null_literals check only catches the
    // exact literal "true"/"false", not an arbitrary bool-named variable.
    const line = "        .excludeSecrets = has_secrets,";
    const sep_pos = secretScanFindAssignmentSep(line).?;
    const val = secretScanExtractValue(line, sep_pos);
    try std.testing.expect(!secretScanValueWasQuoted(line, sep_pos));
    try std.testing.expect(secretScanIsBareIdentifier(val));

    // A real quoted secret is identifier-shaped too (letters/digits/
    // underscore only) -- quoting, not shape, is what distinguishes a
    // literal from a variable reference. Confirm it's still detected as
    // quoted, so the matching loop's `!quoted` guard won't suppress it.
    // (Deliberately not a recognized .prefix pattern like "sk_live_" --
    // this test targets the assignment_key path only.)
    const quoted_line = "        secret_key = \"correcthorsebatterystaple123\",";
    const quoted_sep = secretScanFindAssignmentSep(quoted_line).?;
    try std.testing.expect(secretScanValueWasQuoted(quoted_line, quoted_sep));
}
...
test "computePublicReleaseCheck: regression — a non-git target must never report ready:true" {
    // prcTrackedFiles used to swallow ANY `git ls-files` failure (not a git
    // repo, missing path, etc.) as an empty file list, indistinguishable
    // from "this repo genuinely has zero tracked files" -- every check
    // downstream then scanned nothing and reported ready:true even though
    // the target was full of unscanned real content. `std.testing.tmpDir`
    // nests inside this repo's own `.zig-cache/`, so `git -C` on it walks up
    // and finds this repo's real `.git` -- a nonexistent path is the
    // reliable way to force the git failure regardless of test-runner CWD.
    const gpa = std.testing.allocator;
    const io = std.testing.io;

    const result = try computePublicReleaseCheck(gpa, io, "/nonexistent/not-a-repo-at-all", null);
    defer {
        for (result.findings) |f| {
            gpa.free(f.detail);
            if (f.file.len > 0) gpa.free(f.file);
        }
        gpa.free(result.findings);
    }
