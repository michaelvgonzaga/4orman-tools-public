...
// by a downstream CLI's log pattern library, but works for any log + any
// pattern file.
...
pub fn computeRegistry() RegistryResult {
    const cmds: []const RegistrySubcommand = &[_]RegistrySubcommand{
        ...
        .{ .name = "ledger", .description = "decision ledger — Claude-vs-Zig contests, values trade-offs, and solo-vs-multi-agent approach decisions, with 365-day staleness revalidation (stored at ~/.4orman/ledger.json)", .args = "[show | record <winner> <question> <reasoning> | record-jungian <question> <chosen> <shadow> <synthesis> | record-outcome <id> <outcome> <matched|diverged> | record-approach <question> <solo|multi-agent> <reasoning> <tokens-spent> | check-stale | validate <id> | score <question> <sources-json>]" },
...
fn findLedgerPrecedent(gpa: std.mem.Allocator, io: std.Io, query: []const u8) !?LedgerEntry {
    ...
    for (entries.items, 0..) |e, idx| {
        ...
        if (e.is_stale or std.mem.eql(u8, e.category, "jungian") or std.mem.eql(u8, e.category, "approach")) continue;
...
fn writeLedgerFile(gpa: std.mem.Allocator, io: std.Io, path: []const u8, entries: []const LedgerEntry) void {
    ...
    const ok = blk: {
        ...
        for (entries, 0..) |e, i| {
            ...
            w.interface.print("{{\"id\":\"{s}\",\"date\":\"{s}\",\"recorded_ts\":{d},\"revalidation_due_ts\":{d},\"winner\":\"{s}\",\"question\":\"{s}\",\"reasoning\":\"{s}\",\"is_stale\":{s},\"category\":\"{s}\",\"shadow\":\"{s}\",\"synthesis\":\"{s}\",\"outcome\":\"{s}\",\"outcome_matched\":\"{s}\",\"outcome_recorded_ts\":{d},\"tokens_spent\":{d}}}", .{
                id_esc, date_esc, e.recorded_ts, e.revalidation_due_ts, win_esc, q_esc, r_esc,
                if (e.is_stale) "true" else "false",
                cat_esc, shadow_esc, synth_esc, outcome_esc, matched_esc, e.outcome_recorded_ts, e.tokens_spent,
            }) catch break :blk false;
        }
        w.interface.writeAll("]}\n") catch break :blk false;
        w.interface.flush() catch break :blk false;
