...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len < 2) {
        ...
        try err.print("  ledger [show | record <winner> <question> <reasoning> | record-jungian <question> <chosen> <shadow> <synthesis> | record-outcome <id> <outcome> <matched|diverged> | record-approach <question> <solo|multi-agent> <reasoning> <tokens-spent> | check-stale | validate <id> | score <question> <sources-json>]\n", .{});
    ...
    if (std.mem.eql(u8, args[1], "doctor")) {
        ...
        if (args.len >= 3 and std.mem.eql(u8, args[2], "score")) {
            ...
            for (lresult.entries, 0..) |e, i| {
                ...
                try out.print("\n    {{\"id\": \"{s}\", \"date\": \"{s}\", \"recorded_ts\": {d}, \"revalidation_due_ts\": {d}, \"category\": \"{s}\", \"winner\": \"{s}\", \"question\": \"{s}\", \"reasoning\": \"{s}\", \"shadow\": \"{s}\", \"synthesis\": \"{s}\", \"outcome\": \"{s}\", \"outcomeMatched\": \"{s}\", \"outcomeRecordedTs\": {d}, \"outcomeReviewDue\": {s}, \"tokensSpent\": {d}, \"is_stale\": {s}}}", .{
