...
| M46 — log-match subcommand | Match raw log text against a caller-supplied pattern-library JSON file — domain-agnostic, no vocabulary about any specific log format baked into the binary | `4orman-tools log-match <pattern-file>` (log text on stdin) returns `{"matchCount", "hits": [{id, category, severity, lineNumber, matchedLine, signal, rootCause, conclusion}], "unmatchedSignalLines", "exitStatusLine"}`; used by a consuming project's own pattern library, kept entirely out of this repo — this subcommand is generic, the domain knowledge lives in the caller's data file |
