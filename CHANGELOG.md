...
### New
- `ledger record-feature <domain> <idea-summary> <ready|needs-input|deferred> <tool|doc|project|none> <artifact-ref> <reasoning> <shadow> <synthesis>` — feature-support-ticket ledger category (`"feature"`), for brainstorm/idea evaluation against `/new-project`'s spec-interview rigor. Same reuse-not-new-file pattern as `bug`; `status` derived from `artifact_ref` like `curriculum`.
