...
`4orman-tools log-match <pattern-file>` (log text on stdin)

Domain-agnostic — no vocabulary about any specific log format is in the binary. `<pattern-file>` is a caller-supplied JSON file: `{"patterns": [{"id", "category", "severity", "match": "substring"|"glob", "value", "signal", "rootCause", "conclusion"}]}`. `match: "glob"` supports `*` as a multi-char wildcard within a line; anything else is treated as a literal substring.

```json
{
  "matchCount": 1,
  "hits": [
    {"id": "drupal-missing-module", "category": "application", "severity": "error", "lineNumber": 3, "matchedLine": "...", "signal": "...", "rootCause": "...", "conclusion": "..."}
  ],
  "unmatchedSignalLines": ["[error] ...line with a severity marker but no pattern hit..."],
  "exitStatusLine": "Finished: FAILURE"
}
```

**Constraints:** Reads up to 4 MB from stdin. `unmatchedSignalLines` capped at 20. `exitStatusLine` is the first line containing `Finished:` or `exit code`, or `null`.

---

## ledger

`4orman-tools ledger [show | record <winner> <question> <reasoning> | record-jungian <question> <chosen> <shadow> <synthesis> | record-outcome <id> <outcome> <matched|diverged> | record-approach <question> <solo|multi-agent> <reasoning> <tokens-spent> | check-stale | validate <id> | score <question> <sources-json>]`

Stored at `~/.4orman/ledger.json`, append-only. `category` determines what `winner`/`shadow`/`synthesis` mean — same fields, category-dependent meaning, rather than a column per category:
...
```json
{
  ...
  "stale_count": 0,
  "entries": [
    {"id": "692ccf3c07d09dbb", "date": "2026-07-04", "recorded_ts": 1783144568, "revalidation_due_ts": 1814680568, "category": "approach", "winner": "solo", "question": "...", "reasoning": "...", "shadow": "", "synthesis": "", "outcome": "", "outcomeMatched": "unknown", "outcomeRecordedTs": 0, "outcomeReviewDue": false, "tokensSpent": 4200, "is_stale": false}
  ]
}
```

**Categories:** `rps` (default — factual Claude-vs-Zig contest; `winner` is `"claude"`/`"zig"`) · `jungian` (values trade-off, no winner; `winner` holds the chosen path; `shadow`/`synthesis` populated) · `approach` (solo-vs-multi-agent decision; `winner` is `"solo"`/`"multi-agent"`; `tokensSpent` is measured after the fact, never predicted).

**Constraints:** `jungian` and `approach` entries never resolve tool-choice ambiguity (`capability-check`/`route`/`build`/`run-tests` precedent lookups skip both categories). Stale after 365 days (`revalidation_due_ts`). `outcomeReviewDue` is computed fresh on every read, never persisted. `tokensSpent` defaults to `0` on entries recorded before this field existed.
...
```json
[
  {"name": "example-project", "url": "https://github.com/michaelvgonzaga/example-project", "isForeman": true, "isLocal": true}
]
```
...
```json
{
  ...
  "path": "/Users/user/4orman/example-project",
  "ready": true,
  "captured": [
    { "label": "spec.md present", "source": "/Users/user/4orman/example-project/spec.md" },
    { "label": "CLAUDE.md decision log: 8 rows", "source": "/Users/user/4orman/example-project/CLAUDE.md" }
  ],
  "unextracted": [],
  "warnings": []
}
...
```json
{
  ...
  "version": "v1.3.0",
  "format": "fmz",
  "output_path": "/Users/user/example-project-v1.3.0.fmz",
  "success": true,
  "note": "import on any machine: 4orman-tools import example-project-v1.3.0.fmz"
}
```
...
{
  "name": "example-project",
  "dest_path": "/Users/user/4orman/example-project",
  "source_format": "fmz",
  "deps_note": "run `4orman-tools knowledge-audit` to verify knowledge extraction",
  "success": true,
  "note": "imported → /Users/user/4orman/example-project"
}
```
